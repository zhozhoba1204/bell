...
class OfficeControllerTest {
    ...
    public void filterShouldReturnSuccess() throws Exception {
        OfficeRequestDto officeRequestDto = new OfficeRequestDto(1,"Fest","89097777777", true);
    ...
    public void filterShouldReturnErrorWhenOrgIdIsIncorrect() throws Exception {
        OfficeRequestDto officeRequestDto = new OfficeRequestDto(10,"Fest","89097777777", true);
        mockMvc.perform(post("/api/office/list")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(officeRequestDto)))
    ...
    public void loadByIdShouldReturnSuccess() throws Exception {
        ...
        Office office = OfficeUpdateDto.getOfficeFromUpdateDto(officeUpdateDto);
        mockMvc.perform(
                        get("/api/office/2")
                                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name", is(office.getName())))
                .andExpect(jsonPath("$.data.address", is(office.getAddress())))
                .andExpect(jsonPath("$.data.phone", is(office.getPhone())))
                .andExpect(jsonPath("$.data.isActive", is(office.asActive())));
    }
